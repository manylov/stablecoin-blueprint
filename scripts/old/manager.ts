...
import { changeManager, mint } from "../contracts/jetton-minter";
...
async function manager(manager: string) {
  ...
  await walletContract.sendTransfer({
    ...
    messages: [
      internal({
        ...
        value: "0.1",
        body: changeManager({
          newManager: Address.parse(manager),
        }),
        bounce: false,
      }),
    ],
...
manager(
  "EQD3QyTQddYpmXapaCZDzxf5OiWb0QBMdOZmtvD9AyME_BgJ" // address of new manager contract
);
